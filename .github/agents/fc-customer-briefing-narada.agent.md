...
description: >
  ...
  bug communication for customer, project briefing.
...
3. **Find the RCA report**: Search `.flowcraft/case-files/rca/` for a directory containing the Jira ID. Read `rca-report.md` if found. Extract:
...
3. Re-search `.flowcraft/case-files/rca/` for the new report.
...
1. Locate the RCA directory: `.flowcraft/case-files/rca/<date>--<jira_id>--<slug>/`
   - If it doesn't exist (gap fill created it elsewhere), create a new directory: `.flowcraft/case-files/customer-briefings/<jira_id>/`
